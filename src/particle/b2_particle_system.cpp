...
void b2ParticleSystem::Draw(b2Draw* draw) const
{
	int32 particleCount = GetParticleCount();
	if (particleCount)
	{
		float radius = GetRadius();
		const b2Vec2* positionBuffer = GetPositionBuffer();
		if (m_colorBuffer.data)
		{
			const b2ParticleColor* colorBuffer = GetColorBuffer();
			draw->DrawParticles(positionBuffer, radius, colorBuffer, particleCount);
		}
		else
		{
			draw->DrawParticles(positionBuffer, radius, nullptr, particleCount);
		}
	}
}
...
size_t b2ParticleSystem::LifetimeToExpirationTime(const float lifetime) const
{
	return m_timeElapsed + (size_t)((lifetime / m_def.lifetimeGranularity) *
								   (float)(1LL << 32));
}
								  ...
								  const b2Vec2& force)
{
	...
	const b2Vec2 distributedForce = (1.0f / (float)(lastIndex - firstIndex)) * force;
										  ...
										  const b2Vec2& impulse)
{
	...
	const b2Vec2 velocityDelta = (1.0f / totalMass) * impulse;
	for (int32 i = firstIndex; i < lastIndex; i++)
	{
