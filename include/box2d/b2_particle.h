...
public:
	...
	inline b2ParticleColor(uint8 r, uint8 g, uint8 b, uint8 a)
	{
		Set(r, g, b, a);
	}
	...
	///
	inline void Set(uint8 r_, uint8 g_, uint8 b_, uint8 a_)
	{
		r = r_;
		g = g_;
		b = b_;
	...
	/// overflows will wrap.
	inline b2ParticleColor& operator += (const b2ParticleColor &color)
	{
		r += color.r;
		g += color.g;
		b += color.b;
	...
	inline void Mix(b2ParticleColor * const mixColor, const int32 strength)
	{
	...
	/// in an equal mix of both colors.  strength 0..128 is analogous to an
	/// alpha channel value between 0.0f..0.5f.
	static inline void MixColors(b2ParticleColor * const colorA,
